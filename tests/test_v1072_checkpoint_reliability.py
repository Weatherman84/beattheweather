...
def test_checkpoint_schedule_uses_airport_local_time() -> None:
    schedule = dict(_research_checkpoint_schedule(date(2026, 8, 9), "Europe/Berlin"))
    assert schedule["D-1 @20"].astimezone(timezone.utc) == datetime(
        2026, 8, 8, 18, tzinfo=timezone.utc
    )
    assert schedule["D0 @06"].astimezone(timezone.utc) == datetime(
        2026, 8, 9, 4, tzinfo=timezone.utc
    )
    assert schedule["D0 @10"].astimezone(timezone.utc) == datetime(
        2026, 8, 9, 8, tzinfo=timezone.utc
    )
...
def test_research_collector_reconstructs_exact_cutoff_with_provenance(
    ...
    Base.metadata.create_all(engine)
    session_factory = sessionmaker(engine)
    current_time = datetime(2026, 8, 9, 8, 10, tzinfo=timezone.utc)
    target = date(2026, 8, 9)
    ...
    expected_cutoff = datetime(2026, 8, 9, 8, tzinfo=timezone.utc)
    d0_ten = [
        item
        for item in captured
        if item[1]["checkpoint_label"] == "D0 @10" and item[0] == expected_cutoff
    ]
    assert len(d0_ten) == 1
    assert d0_ten[0][0] == expected_cutoff
    assert d0_ten[0][1]["checkpoint_reconstructed"] is True
    assert "test-model" in str(d0_ten[0][1]["source_provenance_json"])
