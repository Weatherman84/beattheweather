...
if coverage_report_path.exists():
    try:
        ...
        st.sidebar.error("Collection coverage report is unreadable.")
    else:
        relevant_warnings = [
            item
        ...
        historical_notices = [
            item for item in relevant_warnings if not item.get("active", True)
        ]
        if active_warnings:
            st.sidebar.warning(
                f"Data coverage: {len(active_warnings)} active warning(s)."
            )
        elif historical_notices:
            st.sidebar.info(
                f"Data coverage: {len(historical_notices)} historical notice(s); "
                "no current warning."
            )
        if relevant_warnings:
            with st.sidebar.expander("Coverage details"):
                for item in [*active_warnings, *historical_notices][:8]:
                    prefix = "Active" if item.get("active", True) else "Historical"
                    st.write(
                        f"• {prefix}: "
...
if st.sidebar.button("Refresh live trading data", type="primary"):
    before_metar = utc_timestamp(latest_metar_time(airport))
    before_taf = utc_timestamp(latest_taf_time(airport))
    try:
        with st.spinner("Refreshing models, METAR, TAF and Polymarket…"):
            result = collect_live_trading_refresh(airport, target)
    except Exception as exc:
        st.sidebar.error(
            f"Live trading refresh failed ({type(exc).__name__}). The dashboard "
            "remains usable and will retry automatically."
        )
    else:
        # This is a bounded single-airport refresh of the active information set.
        # Persistent journaling remains owned by the single GitHub writer.
        refresh_database_connections()
        init_db()
        after_metar = utc_timestamp(latest_metar_time(airport))
        after_taf = utc_timestamp(latest_taf_time(airport))
        ...
        error_note = (
            " Some sources failed: " + ", ".join(sorted(errors)) + "."
            if errors
            else ""
        )
        refresh_level = (
            "success"
            if not errors and refreshed_models >= requested_models
            else "warning"
        )
        metar_advanced = after_metar is not None and (
            before_metar is None or after_metar > before_metar
        ...
        if metar_advanced:
            ...
            taf_note = " TAF also advanced." if taf_advanced else ""
            feedback = (
                refresh_level,
                f"{saved} METAR advanced to {local_metar}.{taf_note}{error_note}",
            )
        elif taf_advanced:
            local_taf = after_taf.tz_convert(timezone_name).strftime("%d.%m.%Y %H:%M")
            feedback = (
                refresh_level,
                f"{saved} TAF advanced to {local_taf}.{error_note}",
            )
        elif after_metar is not None:
            local_metar = after_metar.tz_convert(timezone_name).strftime("%d.%m.%Y %H:%M")
            feedback = (
                refresh_level,
                f"{saved} The latest METAR remains {local_metar}. The displayed "
                f"information set was reloaded.{error_note}",
            )
        else:
            feedback = (
                "warning",
                f"{saved} No usable METAR was returned. The displayed information set "
                f"was reloaded.{error_note}",
            )
...
with Session() as session:
    ...
    all_forecast_variants = read_archive_live(
...
with tab_shadow:
    ...
    st.caption(
        ...
        "two-percentage-point safety margin. Champion probabilities are not calibrated; "
    ...
    if target_shadow.empty:
        ...
        if "raw_probability" not in target_shadow:
            target_shadow["raw_probability"] = None
        if "forecast_snapshot_at" not in target_shadow:
            target_shadow["forecast_snapshot_at"] = target_shadow["captured_at"]
        target_shadow["forecast_snapshot_at"] = pd.to_datetime(
            target_shadow.forecast_snapshot_at,
            utc=True,
            errors="coerce",
        )
        ...
        shown["captured_at"] = shown.captured_at.dt.tz_convert(
            timezone_name
        ).dt.strftime("%d.%m.%Y %H:%M:%S")
        shown["forecast_snapshot_at"] = (
            shown.forecast_snapshot_at.dt.tz_convert(timezone_name)
            .dt.strftime("%d.%m.%Y %H:%M:%S")
            .fillna("—")
        )
        shown["target_date"] = pd.to_datetime(shown.target_date).dt.strftime(
            "%d.%m.%Y"
        )
        for column in [
            "raw_probability",
        ...
        shown["blockers_json"] = shown.blockers_json.map(
            lambda value: (
                " · ".join(json.loads(value)) or "—"
        ...
        shown = shown.rename(
            columns={
                ...
                "captured_at": f"Captured locally ({timezone_name})",
                "forecast_snapshot_at": f"Forecast snapshot ({timezone_name})",
                "timing": "Information set",
                "bucket_label": "Range",
                "raw_probability": "Raw bucket probability",
                "fair_probability": "Champion probability",
                "best_ask": "Best ask",
        ...
        if not target_baskets.empty:
            ...
            b2.metric(
                "Champion combined probability",
                f"{float(latest_basket.fair_probability):.1%}",
            )
...
with tab_accuracy:
    ...
    checkpoint_health = checkpoint_completeness(
        all_forecast_snapshots,
    ...
    st.caption(
        "Exact local D-1@20, D0@06 and D0@10 captures. Reconstructed rows use only "
        "guidance proven available before the cut-off."
    )
    if checkpoint_health.empty:
        st.info("No fixed checkpoints are due yet.")
    else:
        shown_health = checkpoint_health.copy()
        shown_health["checkpoint_at"] = pd.to_datetime(
            shown_health.checkpoint_at, utc=True
        ).dt.tz_convert(timezone_name).dt.strftime("%d.%m. %H:%M")
...
with tab_data:
    ...
    taf_checkpoint_scored = taf_checkpoint_verification_frame(
        all_tafs,
        station_actuals,
        {code: item["timezone"] for code, item in catalog.items()},
    )
    taf_checkpoint_metrics = taf_verification_metrics(taf_checkpoint_scored)
    st.subheader("TAF TX verification · every official revision")
    ...
    st.subheader("TAF TX verification · actually known at fixed checkpoints")
    selected_checkpoint_metrics = (
        taf_checkpoint_metrics[taf_checkpoint_metrics.airport == airport].copy()
        if not taf_checkpoint_metrics.empty
        else pd.DataFrame()
    )
    if selected_checkpoint_metrics.empty:
        st.caption(
            "No settled checkpoint has a causally available TAF revision yet. Historical "
            "backfills never rewrite this table."
        )
    else:
        for column in ["bias", "mae"]:
            selected_checkpoint_metrics[column] = selected_checkpoint_metrics[column].map(
                lambda value: f"{value:.2f} °C"
            )
        for column in ["exact_hit", "within_1c"]:
            selected_checkpoint_metrics[column] = selected_checkpoint_metrics[column].map(
        ...
        st.dataframe(selected_checkpoint_metrics, hide_index=True, width="stretch")
    if not market_snapshots.empty:
