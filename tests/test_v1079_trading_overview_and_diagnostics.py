...
def test_oos_reliability_uses_stage_specific_scheduled_evidence() -> None:
    ...
    d010 = reliability[reliability.stage == "D0 @10 LT · Champion"].iloc[0]
    live = reliability[
...
def test_ui_scope_and_central_terminology_are_visible() -> None:
    ...
    maintenance = (ROOT / "scripts" / "run_database_job.sh").read_text(encoding="utf-8")
    assert '"All airports", "Airport detail"' in app
    assert 'button("Refresh all airports"' in app
    assert "Champion reliability by checkpoint" in app
    assert "First stored live snapshot after D0@10" in app
    assert "database_maintenance_threshold_bytes=$((35 * 1024 * 1024))" in maintenance
    assert CHECKPOINT_LABELS["d0_10"] == "D0 @10 LT"
