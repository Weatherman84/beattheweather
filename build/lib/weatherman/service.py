...
def _research_checkpoint_schedule(
    ...
    """Return the three canonical airport-local information cut-offs."""
    zone = ZoneInfo(timezone_name)
    return (
        (
            "D-1 @20",
            datetime(target.year, target.month, target.day, 20, tzinfo=zone)
            - timedelta(days=1),
        ),
        (
            "D0 @06",
            datetime(target.year, target.month, target.day, 6, tzinfo=zone),
        ),
        (
            "D0 @10",
            datetime(target.year, target.month, target.day, 10, tzinfo=zone),
        ),
    )
...
def _build_nowcast_from_session(
    ...
    forecasts = read_archive_live(Forecast, connection, filters={"airport": code})
    ...
    hourly = read_archive_live(HourlyForecast, connection, filters={"airport": code})
...
def collect_research_checkpoints(
    ...
    *,
    ...
    D0 accuracy checkpoint remains 10:00. A missed run is reconstructed only
    from guidance whose availability timestamp is at or before the cut-off.
    Full METAR/TAF/Meteoblue collection remains limited to the Trading Desk tier.
    """
    init_db()
    ...
    with Session() as session:
        ...
        for code in selected_codes:
            ...
            for target in targets or set():
                for label, cutoff in _research_checkpoint_schedule(
                    target, airport["timezone"]
                ):
                    cutoff_utc = cutoff.astimezone(timezone.utc)
                    seconds_to_cutoff = (cutoff_utc - current_time).total_seconds()
                    in_capture_window = (
                        -max(1, catchup_hours) * 3600
                        <= seconds_to_cutoff
                        <= max(1, window_minutes) * 60
                    )
            ...
            batches: list[dict] = []
            refresh_due = any(
                cutoff >= current_time - timedelta(minutes=max(45, window_minutes))
                for _target, _label, cutoff in due
            )
            if refresh_due:
                models = airport.get(
                    "research_models",
...
def collect_live_trading_refresh(
    ...
    *,
    ...
    serialized in this process. The GitHub collector stays the only persistent writer;
    this function only refreshes the active Streamlit data copy for an immediate decision.
    """
    init_db()
    started = time.perf_counter()
    catalog = airports()
